...
func init() {
	glsStorage = cache.New(60*time.Second, 60*time.Second)
}

func id() string {
	return fmt.Sprint(curGoroutineID())
}
...
func Shutdown() {
	glsStorage.Delete(id())
}

func get() (map[string]interface{}, bool) {
	data, ok := glsStorage.Get(id())
	if ok {
...
func set(data map[string]interface{}) {
	glsStorage.Set(id(), data, 120*time.Second)
}
